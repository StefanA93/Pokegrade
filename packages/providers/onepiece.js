...
export class OnePieceProvider extends TCGProvider {
  ...
  async fetchAllSets() {
    ...
    const r = await this.timedFetch(`${BASE}/sets?game=one-piece-card-game&limit=200`, { headers: this.headers })
  ...
  async fetchCard(name) {
    ...
    const r = await this.timedFetch(
      `${BASE}/search?q=${encodeURIComponent(name)}&game=one-piece-card-game&limit=1`,
      { headers: this.headers }
    )
    if (!r.ok) return null
    const d     = await r.json()
