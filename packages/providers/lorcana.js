...
export class LorcanaProvider extends TCGProvider {
  ...
  async fetchAllSets() {
    ...
    const r = await this.timedFetch(`${BASE}/sets?game=lorcana`, { headers: this.headers })
  ...
  async fetchCardsForSet(externalSetId) {
    ...
    const r = await this.timedFetch(
      `${BASE}/cards?game=lorcana&set=${encodeURIComponent(externalSetId)}`,
  ...
  async fetchCard(name) {
    ...
    const r = await this.timedFetch(
      `${BASE}/cards?game=lorcana&name=${encodeURIComponent(name)}`,
      { headers: this.headers }
    )
    const d = await r.json()
    const cards = Array.isArray(d) ? d : (d.data || d.cards || [])
  ...
  normalizeCard(c) {
    ...
    return {
      ...
      finishTypes: c.isFoil || c.foil ? ['Normal', 'Foil'] : ['Normal'],
      ...
      metadata:    { color: c.color || null, cost: c.cost ?? null, inkable: c.inkable ?? null }
    }
  }
}
